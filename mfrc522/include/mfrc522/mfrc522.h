...
const std::uint8_t MFRC522_firmware_referenceV1_0[]
    = {0x00, 0xC6, 0x37, 0xD5, 0x32, 0xB7, 0x57, 0x5C, 0xC2, 0xD8, 0x7C, 0x4D, 0xD9,
       0x70, 0xC7, 0x73, 0x10, 0xE6, 0xD2, 0xAA, 0x5E, 0xA1, 0x3E, 0x5A, 0x14, 0xAF,
       0x30, 0x61, 0xC9, 0x70, 0xDB, 0x2E, 0x64, 0x22, 0x72, 0xB5, 0xBD, 0x65, 0xF4,
       0xEC, 0x22, 0xBC, 0xD3, 0x72, 0x35, 0xCD, 0xAA, 0x41, 0x1F, 0xA7, 0xF3, 0x53,
...
public:
    ...
    enum PCD_Register {
        ...
        TxControlReg
        = 0x14u << 1u, // controls the logical behavior of the antenna driver pins TX1 and TX2
        ...
        GsNReg
        = 0x27u << 1u, // selects the conductance of the antenna driver pins TX1 and TX2 for modulation
        CWGsPReg
        = 0x28u << 1u, // defines the conductance of the p-driver output during periods of no modulation
        ModGsPReg
        = 0x29u << 1u, // defines the conductance of the p-driver output during periods of modulation
        TModeReg = 0x2Au << 1u, // defines settings for the internal timer
        TPrescalerReg
        = 0x2Bu << 1u, // the lower 8 bits of the TPrescaler value. The 4 high bits are in TModeReg.
    ...
    enum PICC_Command {
        ...
        PICC_CMD_MF_WRITE
        = 0xA0, // Writes one 16 uint8_t block to the authenticated sector of the PICC. Called "COMPATIBILITY WRITE" for MIFARE Ultralight.
    ...
    /////////////////////////////////////////////////////////////////////////////////////
    void PCD_WriteRegister(uint8_t reg, uint8_t value) const;
    void PCD_WriteRegister(uint8_t reg, uint8_t count, uint8_t *values) const;
    uint8_t PCD_ReadRegister(uint8_t reg) const;
    void PCD_ReadRegister(uint8_t reg, uint8_t count, uint8_t *values, uint8_t rxAlign = 0) const;
    void PCD_SetRegisterBitMask(uint8_t reg, uint8_t mask) const;
    void PCD_ClearRegisterBitMask(uint8_t reg, uint8_t mask) const;
    uint8_t PCD_CalculateCRC(uint8_t *data, uint8_t length, uint8_t *result) const;
    ...
    void PCD_SetAntennaGain(uint8_t mask) const;
    ...
    /////////////////////////////////////////////////////////////////////////////////////
    uint8_t PCD_Authenticate(uint8_t command, uint8_t blockAddr, MIFARE_Key *key, Uid *uid) const;
    ...
    // Support functions
    /////////////////////////////////////////////////////////////////////////////////////
    uint8_t PCD_MIFARE_Transceive(uint8_t *sendData, uint8_t sendLen, bool acceptTimeout = false) const;
